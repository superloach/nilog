...
import (
	...
	"github.com/superloach/nilog/internal"
)
...
var tests = []tester{
	...
	// everything at once:
	{Ldate | Ltime | Lmicroseconds | Llongfile, "XXX", "XXX" + Rdate + " " +
		Rtime + Rmicroseconds + " " + Rlongfile + " "},
	{Ldate | Ltime | Lmicroseconds | Lshortfile, "XXX", "XXX" + Rdate + " " +
		Rtime + Rmicroseconds + " " + Rshortfile + " "},
	{Ldate | Ltime | Lmicroseconds | Llongfile | Lmsgprefix, "XXX", Rdate +
		" " + Rtime + Rmicroseconds + " " + Rlongfile + " XXX"},
	{Ldate | Ltime | Lmicroseconds | Lshortfile | Lmsgprefix, "XXX", Rdate +
		" " + Rtime + Rmicroseconds + " " + Rshortfile + " XXX"},
}
...
func BenchmarkItoa(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		dst = dst[0:0]
		internal.Itoa(&dst, 2015, 4)   // year
		internal.Itoa(&dst, 1, 2)      // month
		internal.Itoa(&dst, 30, 2)     // day
		internal.Itoa(&dst, 12, 2)     // hour
		internal.Itoa(&dst, 56, 2)     // minute
		internal.Itoa(&dst, 0, 2)      // second
		internal.Itoa(&dst, 987654, 6) // microsecond
	}
}
